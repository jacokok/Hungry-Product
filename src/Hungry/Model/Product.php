	...
	*/
	public function get_xml()
	{
		...
		if($this->validate_product_data($p)) {
			...
			$template->condition = get_option('hungry_setting_condition','new');
			...
			$template->product_type = get_option('hungry_setting_type','none') == 'none' ? '' : substr($this->get_type(),0,750);
	...
	private function get_setting($name)
	{
		...
		// Global setting
		$global_setting = get_option('hungry_setting_' . $name);
